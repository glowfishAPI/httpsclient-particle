...
static int32 testUserEc(int32 ecFlags, const sslKeys_t *keys)
{
	...
	if (keys->privKey) {
		if (keys->privKey->type == PS_ECC) {
			...
			if (psTestUserEcID(eccKey->dp->curveId, ecFlags) < 0) {
				return PS_FAILURE;
			}
		}
	}
...
#ifdef USE_RSA
		if (privKeyType == PS_RSA) {
			if ((err = pkcs1ParsePrivBin(pool, (unsigned char*)privBuf,
					(uint32)privLen, &keys->privKey)) < 0) {
#ifdef USE_PKCS8
...
#ifdef USE_ECC
		if (privKeyType == PS_ECC) {
			if ((err = psEcdsaParsePrivKey(pool, (unsigned char*)privBuf,
					(uint32)privLen, &keys->privKey, NULL)) < 0) {
#ifdef USE_PKCS8
				/* Attempt a PKCS#8 but mem parse doesn't take password */
				if ((err = pkcs8ParsePrivBin(pool, (unsigned char*)privBuf,
						(uint32)privLen, NULL, &keys->privKey)) < 0) {
...
*/
	...
	if ((ssl->flags & SSL_FLAGS_SERVER) && ssl->sid) {
		/* No allocated sub-members possible on server side? */
...
int32 matrixCreateSessionTicket(ssl_t *ssl, unsigned char *out, int32 *outLen)
{
	...
	*c = ticketLen & 0xFF; c++;
	...
	if (matrixCryptoGetPrngData(randno, 16, ssl->userPtr) < 0) {
