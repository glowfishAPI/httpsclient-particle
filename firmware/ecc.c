/**
 *	@file    ecc.c
 *	@version 33ef80f (HEAD, tag: MATRIXSSL-3-7-2a-OPEN, tag: MATRIXSSL-3-7-2a-COMM, origin/master, origin/HEAD, master)
 *
...
*/
	...
	if ((uint32)(end - p) < 1 || (*(p++) != ASN_BIT_STRING) ||
	...
	if (psEccX963ImportKey(pool, p, arcLen, pubKey, eccSet) < 0) {
		...
		void *pkiData)
{
	...
	if ((err = pstm_read_radix(pool, &order, (char *)key->dp->order, slen, 16))
			!= PS_SUCCESS) {
		pstm_clear(&order);
		goto ERR_BUF;
	}
...
RETRY_RAND:
	if (matrixCryptoGetPrngData(buf, keysize, pkiData) != keysize) {
...
*/
	...
	if ((uint32)(end - buf) < 1) {
		psTraceCrypto("Bad ecc private key len\n");
		return PS_FAILURE;
	}
...
*/
		...
		/* Note len can be 0 here */
		if (psEccX963ImportKey(pool, buf, len, key, key->dp) < 0) {
			psTraceCrypto("Unable to parse ECC pubkey from cert\n");
			psFreePubKey(*pubkey); *pubkey = NULL;
			return -1;
		}
...
/* Return a list of all supported curves.  Will put the weakest first in
		this mode */
...
/* User set list of curves they want to support.  This method will put the
	strongest first in the list */
void userSuppliedEccList(char *curveList, uint32 *len, int32 curves)
{
	psEccSet_t *set;
	uint32	listLen = 0;

	if (curves & IS_SECP521R1) {
		if (getEccParamById(25, &set) == 0) {
			if (listLen < (*len - 2)) {
				curveList[listLen++] = (set->curveId & 0xFF00) >> 8;
				curveList[listLen++] = set->curveId & 0xFF;
			}
		}
	}
	if (curves & IS_SECP384R1) {
		if (getEccParamById(24, &set) == 0) {
			if (listLen < (*len - 2)) {
				curveList[listLen++] = (set->curveId & 0xFF00) >> 8;
				curveList[listLen++] = set->curveId & 0xFF;
			}
							...
							uint32 inlen, psEccKey_t *key, psEccSet_t *dp)
{
	...
	switch(*in) {
	/*	4 is the standard Octet-String id.
		3 has been seen in Bit-String use but should parse just the same. */
	case 4:
	case 3:
		break;
	/* 6 and 7 are for compressed point formats, unsupported currently */
	case 6:
	case 7:
		psTraceCrypto("ERROR: ECC compressed formats unsupported\n");
	default:
	...
	if (dp != NULL && dp->isOptimized) {
		if ((err = pstm_init_for_read_unsigned_bin(pool, &prime, dp->size))
				!= PS_SUCCESS) {
			goto error;
		}
		if ((err = pstm_init_for_read_unsigned_bin(pool, &b, dp->size))
				!= PS_SUCCESS) {
			pstm_clear(&prime);
			goto error;
		}
		if ((err = pstm_read_radix(pool, &prime, (char *)dp->prime,
				dp->size * 2, 16)) != PS_SUCCESS){
			pstm_clear(&prime);
			pstm_clear(&b);
			goto error;
		}

		if ((err = pstm_read_radix(pool, &b, (char *)dp->B,	dp->size * 2, 16))
				!= PS_SUCCESS){
			pstm_clear(&prime);
			pstm_clear(&b);
			goto error;
			...
			void *eccData)
{
	...
	if (public_key->dp != NULL) {
		if (private_key->dp->curveId != public_key->dp->curveId) {
			return PS_ARG_FAIL;
		}
	}
				  ...
				  int32 *bytesWritten, int32 includeSize, void *eccData)
{
	...
	int32			rflag, sflag, sanity, shift;
	rflag = sflag = sanity = 0;
	...
	if ((shift = (privKey->dp->size * sizeof(pstm_digit)) % DIGIT_BIT) == 0) {
		shift = DIGIT_BIT;
	}
	if ((r.dp[r.used - 1] >> (shift - 1)) & 0x1) {
		rLen++;
		rflag = 1;
	}
	if ((s.dp[s.used - 1] >> (shift - 1)) & 0x1) {
		sLen++;
		sflag = 1;
	}
	sigLen = 6 + rLen + sLen;
	...
	*c = rLen & 0xFF; c++;
	...
	c += rLen - rflag;  /* Moved forward rflag already */
	...
	*c = sLen & 0xFF; c++;
	...
	c += sLen - sflag;  /* Moved forward sflag already */
