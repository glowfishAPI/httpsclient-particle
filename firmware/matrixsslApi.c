...
int32 matrixSslNewClientSession(ssl_t **ssl, const sslKeys_t *keys,
...
#ifdef USE_ECC
	/* Must persist.  Set to compiled-in set if not provided by user */
	if (options->ecFlags <= 0) {
		lssl->ecFlags = compiledInEcFlags();
	} else {
		lssl->ecFlags = options->ecFlags;
	}
#endif
	...
	if (ssl->inlen > 0) {
		...
		if (ssl->flags & SSL_FLAGS_GMAC_R) {
			...
			/* TODO: This is for checking async or not.  If async, this length
				tweak never happen.  Need a more generic way to look for
				blocking or not */
